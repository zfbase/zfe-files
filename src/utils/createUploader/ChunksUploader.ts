...
export class ChunksUploader {
  ...
  constructor(private options: ChunksUploaderOptions) {
    this.chunkSize = options.chunkSize ?? 1024 * 1024;
    this.maxThreads = options.maxThreads ?? 1;

    this.chunksCount = Math.ceil(this.options.file.size / this.chunkSize);
  ...
  sendNext() {
    ...
    const chunk = this.options.file.slice(sentSize, sentSize + this.chunkSize);
    ...
    return new Promise<UploadResult>((resolve, reject) => {
      ...
      xhr.send(
        createFormData({
          ...
          fileSize: this.options.file.size.toString(),
          chunksCount: this.chunksCount.toString(),
          chunkNum: chunkNum.toString(),
          uid: this.uploadId,
        }),
      );
    });
